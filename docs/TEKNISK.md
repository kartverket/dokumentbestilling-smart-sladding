...
For hvert godkjente funn beregnes en sladdeboks som dekker de fem siste sifrene, personnummerdelen, slik at fødselsdatoen forblir leselig mens den identifiserende delen skjules. Ble siden rotert før analysen, regnes koordinatene til slutt tilbake til dokumentets opprinnelige orientering, og resultatet returneres som JSON med posisjon, kilde og eventuell konfidensverdi per boks. Konfidens rapporteres som to atskilte felt: `yolo_conf` er bildemodellens deteksjonssikkerhet, mens `paddle_rec_score` er hvor sikkert OCR-en leste tegnene. De måler ulike ting og må ikke slås sammen — en høy lesekvalitet sier ingenting om hvor sikker deteksjonen er.

Bokser som kommer fra bildesporet alene (kilde «yolo») får i tillegg et `trekk`-objekt som beskriver hva OCR-en så i og rundt området: antall siffer og bokstaver, lesekvalitet, lengste sammenhengende sifferløp, om linjen inneholder et ellevesifret løp med gyldig fødselsnummerform, og om tallet har desimalskille. Feltene påvirker ikke sladdingen. De skrives til resultat-CSV-en slik at strengere varianter av innholdssjekken kan måles mot fasit i `utils/filter_sweep.py` uten å kjøre modellen på nytt. Se `app/boks_trekk.py`.
