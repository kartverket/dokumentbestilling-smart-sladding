...
def _hent_reader():
    ...
    if reader is None:
        ...
        print(f"[OCR] enhet: {'GPU (CUDA)' if gpu else 'CPU'}")
...
# === OCR -> bokser ===
def les_sider(sti):
    "Leser alle sider i en PDF til PIL-bilder."
    dokument = fitz.open(sti)
    sider = []
    for side in dokument:
        pixmap = side.get_pixmap(dpi=PDF_DPI)
        png_bytes = pixmap.tobytes("png")
        sider.append(Image.open(io.BytesIO(png_bytes)).convert("RGB"))
    dokument.close()
    return sider
...
def behandle_fil(fil):
    navn = os.path.basename(fil)
    try:
        rader = []
        for si, bilde in enumerate(les_sider(fil), start=1):
            bokser = [b for b, _mod11 in finn_bokser(bilde)]    # bare koordinatene
            rader.append((navn, si, bilde.size[0], bilde.size[1], bokser))
        return navn, rader, None
    except Exception as e:                                      
        return navn, None, repr(e)


def behandle_alle(filer):
    sladd_bokser, feilet = {}, []
    for fil in filer:
        navn, rader, feil = behandle_fil(fil)
        if feil:
            feilet.append((navn, feil))
            continue
        for (navn, si, bw, bh, bokser) in rader:
            sladd_bokser[(navn, si)] = (bw, bh, bokser)
    return sladd_bokser, feilet
